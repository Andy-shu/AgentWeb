...
public class AgentWebFragment extends Fragment implements FragmentKeyDown {
  ...
  public static AgentWebFragment getInstance(Bundle bundle) {
    ...
    if (bundle != null) mAgentWebFragment.setArguments(bundle);
  ...
  public void onViewCreated(View view, @Nullable Bundle savedInstanceState) {
    ...
    mAgentWeb = AgentWeb.with(this)//
        ...
        .setNotifyIcon(R.mipmap.download).createAgentWeb()// 创建AgentWeb
    ...
    DefaultMsgConfig.DownLoadMsgConfig mDownLoadMsgConfig =
        mAgentWeb.getDefaultMsgConfig().getDownLoadMsgConfig();
  ...
  protected ChromeClientCallbackManager.ReceivedTitleCallback mCallback =
      new ChromeClientCallbackManager.ReceivedTitleCallback() {
        @Override
        public void onReceivedTitle(WebView view, String title) {
          if (mTitleTextView != null && !TextUtils.isEmpty(title))
            if (title.length() > 10) title = title.substring(0, 10).concat("...");
  ...
  protected WebViewClient mWebViewClient = new WebViewClient() {
    ...
    public boolean shouldOverrideUrlLoading(final WebView view, String url) {
      ...
      // 优酷想唤起自己应用播放该视频 ， 下面拦截地址返回 true 则会在应用内 H5 播放 ，禁止优酷唤起播放该视频， 如果返回 false ， DefaultWebClient
      // 会根据intent 协议处理 该地址 ， 首先匹配该应用存不存在 ，如果存在 ， 唤起该应用播放 ， 如果不存在 ， 则跳到应用市场下载该应用 .
      if (url.startsWith("intent://") && url.contains("com.youku.phone")) return true;
      /*
       * else if (isAlipay(view, url)) //1.2.5开始不用调用该方法了 ，只要引入支付宝sdk即可 ， DefaultWebClient
       * 默认会处理相应url调起支付宝 return true;
       */
  ...
  public void onActivityResult(int requestCode, int resultCode, Intent data) {
    ...
    mAgentWeb.uploadFileResult(requestCode, resultCode, data); // 2.0.0开始 废弃该api ，没有api代替 ,使用
                                                               // ActionActivity 绕过该方法 ,降低使用门槛
  }
      ...
      new PopupMenu.OnMenuItemClickListener() {
        ...
        public boolean onMenuItemClick(MenuItem item) {
          ...
          switch (item.getItemId()) {
            ...
            case R.id.refresh:
              if (mAgentWeb != null) mAgentWeb.getLoader().reload();
              return true;

            case R.id.copy:
              if (mAgentWeb != null) toCopy(AgentWebFragment.this.getContext(),
