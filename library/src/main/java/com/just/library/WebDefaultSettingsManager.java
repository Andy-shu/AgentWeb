...
public class WebDefaultSettingsManager implements AgentWebSettings, WebListenerManager {
  ...
  private void settings(WebView webView) {
    ...
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN) {
      mWebSettings.setAllowFileAccessFromFileURLs(false); // 通过 file url 加载的 Javascript 读取其他的本地文件
                                                          // .建议关闭
    }
    mWebSettings.setAllowUniversalAccessFromFileURLs(false);// 允许通过 file url 加载的 Javascript
                                                            // 可以访问其他的源，包括其他的文件和 http，https 等其他的源
    ...
    LogUtils.i(TAG,
        "dir:" + dir + "   appcache:" + AgentWebConfig.getCachePath(webView.getContext()));
