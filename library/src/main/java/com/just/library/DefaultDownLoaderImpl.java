...
/**
 * Created by cenxiaozhong on 2017/5/13. source CODE https://github.com/Justson/AgentWeb
 */
...
public class DefaultDownLoaderImpl implements DownloadListener, DownLoadResultListener {
  ...
  public void setParallelDownload(boolean isOpen) {
    ...
    /*
     * if(isParallelDownload.get()){ ExecutorProvider.getInstance().open(); }else{
  ...
  @Override
  public synchronized void onDownloadStart(String url, String userAgent, String contentDisposition,
  ...
  private void onDownloadStartInternal(String url, String contentDisposition, String mimetype,
      long contentLength) {
  ...
  private ActionActivity.PermissionListener getPermissionListener() {
    return new ActionActivity.PermissionListener() {
      ...
      public void onRequestPermissionsResult(@NonNull String[] permissions,
          @NonNull int[] grantResults, Bundle extras) {
        if (checkNeedPermission().isEmpty()) {
          preDownload(DefaultDownLoaderImpl.this.url, DefaultDownLoaderImpl.this.contentDisposition,
              DefaultDownLoaderImpl.this.contentLength);
          url = null;
  ...
  private List<String> checkNeedPermission() {
    ...
    for (int i = 0; i < AgentWebPermissions.STORAGE.length; i++) {
      ...
      if (ContextCompat.checkSelfPermission(mActivityWeakReference.get(),
          AgentWebPermissions.STORAGE[i]) != PackageManager.PERMISSION_GRANTED) {
  ...
  private void preDownload(String url, String contentDisposition, long contentLength) {
    ...
    if (mFile.exists() && mFile.length() >= contentLength) {
      ...
      try {
        ...
        if (mIntent != null) {
          if (!(mContext instanceof Activity)) mIntent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
          mContext.startActivity(mIntent);
        }
        return;
      } catch (Throwable throwable) {
        if (LogUtils.isDebug()) throwable.printStackTrace();
      }
  ...
  private void showDialog(final String url, final long contentLength, final File file) {
    ...
    Activity mActivity;
    if ((mActivity = mActivityWeakReference.get()) == null || mActivity.isFinishing()) return;
  ...
  private void performDownload(String url, long contentLength, File file) {
    ...
    if (isParallelDownload.get()) {
      new RealDownLoader(new DownLoadTask(NoticationID++, url, this, isForce, enableIndicator,
          mContext, file, contentLength, mDownLoadMsgConfig, icon == -1 ? R.mipmap.download : icon))
              .executeOnExecutor(ExecutorProvider.getInstance().provide(), (Void[]) null);
    } else {
      // 默认串行下载.
      new RealDownLoader(new DownLoadTask(NoticationID++, url, this, isForce, enableIndicator,
          mContext, file, contentLength, mDownLoadMsgConfig, icon == -1 ? R.mipmap.download : icon))
  ...
  private File getFile(String contentDisposition, String url) {
    ...
    try {
      ...
      if (!TextUtils.isEmpty(contentDisposition) && contentDisposition.contains("filename")
          && !contentDisposition.endsWith("filename")) {
      ...
      if (TextUtils.isEmpty(filename) && !TextUtils.isEmpty(url) && !url.endsWith("/")) {
        ...
        if (p != -1) filename = url.substring(p + 1);
        if (filename.contains("?")) {
  ...
  static class ExecutorProvider implements Provider<Executor> {
    ...
    private final ThreadFactory sThreadFactory = new ThreadFactory() {
      ...
      private ThreadGroup group = securityManager != null
      ...
      public Thread newThread(Runnable r) {
        ...
        LogUtils.i(TAG,
            "live:" + mThreadPoolExecutor.getActiveCount() + "    getCorePoolSize:"
                + mThreadPoolExecutor.getCorePoolSize() + "  getPoolSize:"
  ...
  public static class ExecuteTasksMap extends ReentrantLock {
    ...
    static ExecuteTasksMap getInstance() {
      ...
      if (sInstance == null) {
        synchronized (ExecuteTasksMap.class) {
          if (sInstance == null) sInstance = new ExecuteTasksMap();
        }
      }
      return sInstance;
    }
    ...
    void removeTask(String path) {
      ...
      try {
        lock();
        int position = -1;
        if ((position = mTasks.indexOf(path)) == -1) return;
  ...
  public static class Builder {
    ...
    public Builder setDownLoadResultListeners(
        List<DownLoadResultListener> downLoadResultListeners) {
