...
public class AgentWebUtils {
  ...
  static final void clearWebView(WebView m) {
    ...
    m.stopLoading();
    if (m.getHandler() != null) m.getHandler().removeCallbacksAndMessages(null);
    m.removeAllViews();
    ViewGroup mViewGroup = null;
    if ((mViewGroup = ((ViewGroup) m.getParent())) != null) mViewGroup.removeView(m);
  ...
  static String getAgentWebFilePath(Context context) {
    if (!TextUtils.isEmpty(AGENTWEB_FILE_PATH)) return AGENTWEB_FILE_PATH;
  ...
  static Uri getUriFromFile(Context context, File file) {
    ...
    // LogUtils.i("Info", "::" + context.getApplicationInfo().targetSdkVersion + " INT:" +
  ...
  static Uri getUriFromFileForN(Context context, File file) {
    Uri fileUri = FileProvider.getUriForFile(context,
        context.getPackageName() + ".AgentWebFileProvider", file);
    // LogUtils.i(TAG, "getUriFromFileForN:" + (context.getPackageName() + ".AgentWebFileProvider"+"
    // uri:"+fileUri));
    return fileUri;
  }


  static void setIntentDataAndType(Context context, Intent intent, String type, File file,
      boolean writeAble) {
  ...
  static void show(View parent, CharSequence text, int duration, @ColorInt int textColor,
      @ColorInt int bgColor, CharSequence actionText, @ColorInt int actionTextColor,
  ...
  static boolean isOverriedMethod(Object currentObject, String methodName, String method,
      Class... clazzs) {
    LogUtils.i(TAG, "  methodName:" + methodName + "   method:" + method);
    boolean tag = false;
    if (currentObject == null) return tag;
    try {
  ...
  static void clearWebViewAllCache(Context context, WebView webView) {
    ...
    try {
      ...
      if (AgentWebConfig.DEBUG) ignore.printStackTrace();
    }
  }
  ...
  static String[] uriToPath(Activity activity, Uri[] uris) {
    ...
    try {
      ...
      int i = 0;
      for (Uri mUri : uris) {
        paths[i++] = Build.VERSION.SDK_INT > Build.VERSION_CODES.JELLY_BEAN_MR2
            ? getFileAbsolutePath(activity, mUri)
  ...
  static class EncodeFileRunnable implements Runnable {
    ...
    public EncodeFileRunnable(String filePath, Queue<FileParcel> queue,
        CountDownLatch countDownLatch, int id) {
    ...
    public void run() {
      ...
      try {
        ...
        if (mFile.exists()) {
          ...
          mQueue.offer(new FileParcel(id, mFile.getAbsolutePath(),
  ...
  static String getFileAbsolutePath(Activity context, Uri fileUri) {
    ...
    LogUtils.i(TAG,
        "getAuthority:" + fileUri.getAuthority() + "  getHost:" + fileUri.getHost() + "   getPath:"
            + fileUri.getPath() + "  getScheme:" + fileUri.getScheme() + "  query:"
            + fileUri.getQuery());
    ...
    else if (fileUri.getAuthority()
        .equalsIgnoreCase(context.getPackageName() + ".AgentWebFileProvider")) {

      String path = fileUri.getPath();
      int index = path.lastIndexOf("/");
      return getAgentWebFilePath(context) + File.separator
  ...
  static boolean isMainProcess(Context context) {
    ...
    if (packgeName.equals(processName)) tag = true;
  ...
  static String md5(String str) {
    try {
      ...
      if (LogUtils.isDebug()) e.printStackTrace();
    }
    return "";
  }
}
